...
/*
Imported from https://github.com/mliebelt/PgnViewerJS
Adapted by Laurent Bernabe
*/
...
var pgnReader = function (configuration, chess) {
    ...
    var load_pgn = function () {
        ...
        // Laurent Bernabe
        // If a position is defined in the headers, then use it.
        var definedPositionInHeader = that.headers.FEN;
        if (definedPositionInHeader) {
            that.configuration.position = that.headers.FEN;
            that.finalPosition = that.headers.FEN;
        }

        // Laurent Bernabe
        // If no move defined, skip this step
        // (A starting position may have been already defined)
        var restString = restString.trim();
        if (restString.length > 0 && restString !== '*') readMoves(restString);
    ...
    var readHeaders = function () {
        ...
        var splitHeaders = function (string) {
            ...
            while (xarr = re.exec(string)) {
                /*
                (Laurent Bernabe) : Bug fixed => the value could be empty ("")
                */
    ...
    var eachMove = function(movesMainLine) {
        ...
        var eachMoveVariation = function(moveArray, level, prev) {
            ...
            utils.pvEach(moveArray, function(move, i) {
                ...
                // Laurent Bernabe : ensuring we have a way to retrieve the final position
                that.finalPosition = fen;
